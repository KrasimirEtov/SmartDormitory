...
namespace SmartDormitory.Services.Utils
{
	public static class Validator
	{
		...
		public static void ValidateSensorValues(float userMinRange, float apiMinRange,
			float userMaxRange, float apiMaxRange, int userRefreshRate, int apiRefreshRate)
		{
			if (userMinRange < apiMinRange || userMaxRange < apiMaxRange
				|| userRefreshRate < apiRefreshRate)
			{
				throw new ArgumentOutOfRangeException("Invalid sensor values");
			}
		}
    }
}
